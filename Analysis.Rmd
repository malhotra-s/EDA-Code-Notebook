...
Since Covid brought its large-scale impact in 2020, people might be interested in what how the distribution might be affected by Covid. The following plot depicts the distribution of police shootings by gender across the country in 2019 before Covid happened. 
```{r warning=FALSE}
all_police_shootings %>% filter(!is.na(longitude) & !is.na(latitude) & gender != "" & year == 2019) %>%
  ggplot(aes(x = longitude, y = latitude, color = gender)) + geom_point(alpha=0.6, size = 0.7) + 
  scale_color_manual(values = c("#EF6F6A", "#1170AA")) + ggtitle("Geographic Distribution by Gender")
```
In contrast this plot displays the police shooting distribution after Covid started in 2020. 
```{r warning=FALSE}
...
In addition to demographics, examining the role of mental illness in these incidents is of paramount importance. Understanding how mental health issues intersect with police shootings can help develop strategies for de-escalation and providing appropriate resources. Equally significant is the evaluation of factors such as the presence of weapons and the use of body cameras during encounters with law enforcement. These elements may shed light on the escalation or de-escalation of confrontations and their impact on outcomes.
