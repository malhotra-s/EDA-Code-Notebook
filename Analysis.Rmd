...
*``race``: [NEED DOUBLE CHECK !!] The race variable categorizes the individual's racial or ethnic background, recorded as "A" for Asian, "B" for Black, "H" for Hispanic, "N" for Native American, "W" for Whites and "O" for others.  
...
distribution of ways of fleeing across different race groups
```{r}
...
From the scatter plot, we can see how  people involved are geographically distributed by gender. Regardless of gender, most incidents concentrate to the eastern region and border/coastal region. More females in the east are involved than in the west
```{r warning=FALSE}
...
table(all_police_shootings$year)
```
...
# how multiple weapons affect police shootings
all_police_shootings %>%
  ggplot(aes(x = multiple_weapon, fill = manner_of_death)) + geom_bar()
# -------------------------
###  Ideas & draft
# unarmed people's age, gender and race
all_police_shootings %>% filter(armed =="unarmed") %>%
  ggplot(aes(x = body_camera, fill = race)) + geom_bar() # + scale_color_manual(values = c("#55AD89", "#EF6F6A", "#1170AA", "#FFD700", "#8566AA", "#FFA500"))


all_police_shootings %>%
  ggplot(aes(x = gender, fill = manner_of_death)) + geom_bar()
